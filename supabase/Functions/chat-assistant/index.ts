...
async function buildPortfolioContext() {
  const [profileRows, skills, certificates, projects, posts] = await Promise.all([
    fetchTable('profile?select=*&limit=1'),
    fetchTable('skills?select=name,category,is_featured&order=sort_order'),
    fetchTable('certificates?select=title,issuer,issue_date&order=sort_order'),
    fetchTable('projects?select=title,description,tech_stack,live_url,repo_url,is_featured&order=sort_order'),
    fetchTable('blog_posts?select=title,excerpt,slug,tags,created_at&is_published=eq.true&order=created_at.desc&limit=25'),
  ]);
  const profile = profileRows[0] || {};

  const lines: string[] = [];
  lines.push(`Nama: ${profile.full_name || 'Tidak diketahui'}`);
  lines.push(`Peran: ${(profile.role_titles || []).join(', ') || 'Tidak ditentukan'}`);
  ...
  lines.push(`GitHub: ${profile.github_username ? `github.com/${profile.github_username}` : 'Tidak ditentukan'}`);

  lines.push('\nSKILLS:');
  if (skills.length === 0) lines.push('(belum ada data)');
  for (const s of skills) lines.push(`- ${s.name} (${s.category}${s.is_featured ? ', unggulan' : ''})`);

  lines.push('\nSERTIFIKAT:');
  if (certificates.length === 0) lines.push('(belum ada data)');
  for (const c of certificates) {
    lines.push(`- ${c.title}, diterbitkan oleh ${c.issuer}${c.issue_date ? ` (${c.issue_date})` : ''}`);
  }
  ...
  for (const b of posts) {
    lines.push(`- "${b.title}" (/blog/${b.slug}): ${b.excerpt || ''}${b.tags?.length ? ` [tag: ${b.tags.join(', ')}]` : ''}`);
  }
...
1. Kamu HANYA membahas ${name}: latar belakang, skill, sertifikat, proyek, artikel blog, dan cara menghubunginya. Basa-basi ringan yang ditujukan ke "Dimdim" (sapaan, "kamu siapa", "bisa bantu apa") boleh dijawab.
...
async function checkAndBumpRateLimit(ip: string): Promise<boolean> {
  ...
  const res = await db(`chat_rate_limits?ip=eq.${encodeURIComponent(ip)}&day=eq.${day}&select=count`);
...
async function requestGemini(
  ...
  if (includeThinking) generationConfig.thinkingConfig = { thinkingLevel: 'LOW' };
...
async function callGemini(systemInstruction: string, contents: unknown[]) {
  ...
  for (const model of GEMINI_MODELS) {
    let res = await requestGemini(model, systemInstruction, contents, true);

    if (!res.ok && res.status === 400) {
      const peek = await res.clone().text();
      if (/thinking/i.test(peek)) {
        console.error(`Model ${model} rejected thinkingConfig, retrying without it`, peek);
        res = await requestGemini(model, systemInstruction, contents, false);
      }
    }
    ...
    if (res.ok) {
      ...
      const reply = candidate?.content?.parts?.map((p: { text?: string }) => p.text || '').join('');
      if (reply && candidate?.finishReason === 'MAX_TOKENS') {

        console.error(`Gemini reply truncated at MAX_TOKENS (model=${model})`);
        return reply.trim() + '\n\n(Masih ada lagi, tapi jawabannya kepanjangan. Tanya lebih spesifik ya biar aku bisa jelasin detail bagian itu.)';
      }
      if (reply) return reply;
      lastError = `Model ${model} mengembalikan jawaban kosong (finishReason: ${candidate?.finishReason || 'tidak diketahui'})`;
      continue;
    }

    const errText = await res.text();
    console.error(`Gemini error (model=${model})`, res.status, errText);
    lastError = errText;
...
Deno.serve(async (req: Request) => {
  ...
  if (message.length > 1200) {
    return json({ reply: 'Pesannya kepanjangan buat aku proses, coba dipersingkat ya.' });
  }
  ...
  if (!allowed) {
    return json({
      reply: 'Aku sudah kehabisan jatah chat untuk hari ini. Coba lagi besok, atau hubungi langsung lewat bagian kontak ya!',
    });
  }

  if (!GEMINI_API_KEY) {
    return json({ reply: 'Dimdim belum diatur sepenuhnya. Pemilik situs perlu menambahkan API key Gemini dulu.' });
  }
  ...
  try {
    ...
    console.error(err);
    return json({ reply: 'Aku lagi ada gangguan teknis. Coba tanya lagi sebentar ya.' });
  }
});